...
public class AnonymousFunction extends Expression {
	...
	private void writeClosureContext(StringBuilder supportBody,
			CompileContext cc) {
		if (freeVariables.size() > 0) {
			supportBody.append("typedef struct GC_STRUCT_CLOSURE_CONTEXT_"
					+ methodIdx + " {\n");
			for (NameTypePair ntp : freeVariables) {
				supportBody.append(Constants.TAB.get()
						+ ntp.type.getCTypeString() + " " + ntp.name + ";\n");
			}
	...
	private String writeContextBuilder(StringBuilder supportBody) {
		...
		supportBody.append(functionType.get().getCTypeString()
				+ " build_new_closure_" + methodIdx + "(" + args + ") {\n");
		supportBody.append(TAB + functionType.get().getCTypeString()
				+ " closure;\n");
		if (freeVariables.size() > 0) {
			supportBody.append(TAB + "GC_STRUCT_CLOSURE_CONTEXT_" + methodIdx
					+ "* context=(GC_STRUCT_CLOSURE_CONTEXT_" + methodIdx
					+ "*) malloc(sizeof(GC_STRUCT_CLOSURE_CONTEXT_" + methodIdx
					+ "));\n");
			for (NameTypePair ntp : freeVariables) {
				supportBody.append(TAB + "context->" + ntp.name + "="
						+ ntp.name + ";\n");
			}
		...
		supportBody.append(TAB + "closure.func=" + "support_method_"
				+ methodIdx + ";\n");
	...
	public Type getTypeHard(TypingEnvironment e) throws TypeException {
		...
		functionType.set(ft);
		FunctionDefinition fdef = new FunctionDefinition("support_method_"
				+ methodIdx, returnType.get());
		...
		e.unwatch();
		e.popScope();
		fdef = new FunctionDefinition("support_method_" + methodIdx,
				returnType.get());
		fdef.add("__CTX", new PointerType(new CoreTypes.VoidType()));
		for (NameTypePair ntp : args) {
			fdef.add(ntp.name, ntp.type);
		...
		Type t = new PointerType(new StructReferenceType("CLOSURE_CONTEXT_"
				+ methodIdx));
		augmentedBody.add(new DeclareType(t, "__CTX_REAL").setValue(new CastAs(
				new Variable("__CTX"), t)));
		for (NameTypePair ntp : freeVariables) {
			augmentedBody
					.add(new DeclareType(ntp.type, ntp.name)
							.setValue(new MemberReference(
									new Variable("__CTX_REAL"),
									ntp.name,
									MemberReference.HowReference.INDIRECT_VIA_POINTER)));
		}

...
public class AnonymousFunction extends Expression {
  ...
  private void writeClosureContext(StringBuilder supportBody, CompileContext cc) {
    if (freeVariables.size() > 0) {
      ...
      for (NameTypePair ntp : freeVariables) {
        supportBody.append(CompileContext.TAB + ntp.type.getCTypeString() + " " + ntp.name + ";\n");
      }
  ...
  private String writeContextBuilder(StringBuilder supportBody) {
    ...
    supportBody.append(CompileContext.TAB + functionType.get().getCTypeString() + " closure;\n");
    if (freeVariables.size() > 0) {
      supportBody.append(CompileContext.TAB + "GC_STRUCT_CLOSURE_CONTEXT_" + methodIdx + "* context=(GC_STRUCT_CLOSURE_CONTEXT_" + methodIdx + "*) malloc(sizeof(GC_STRUCT_CLOSURE_CONTEXT_" + methodIdx + "));\n");
      for (NameTypePair ntp : freeVariables) {
        supportBody.append(CompileContext.TAB + "context->" + ntp.name + "=" + ntp.name + ";\n");
      }
      supportBody.append(CompileContext.TAB + "closure.context=(void*)context;\n");
    } else {
      supportBody.append(CompileContext.TAB + "closure.context=0;\n");
    }
    supportBody.append(CompileContext.TAB + "closure.func=" + "support_method_" + methodIdx + ";\n");
    supportBody.append(CompileContext.TAB + "return closure;\n");
    supportBody.append("}\n");
